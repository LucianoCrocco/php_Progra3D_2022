...
class Usuario{
    ...
    public static function ListarUsuariosHTMLSQL($array){
        $mensaje = "<ul>".PHP_EOL;

        foreach ($array as $v){
            $mensaje = $mensaje."<li>".$v["nombre"]." ".$v["apellido"]." ".$v["clave"]." ".$v["mail"]." ".$v["localidad"]." ".$v["fecha_registro"]."</li>".PHP_EOL;
        }
    ...
    public static function BuscarUsuariosExistenteSQL($mail, $clave, $matrizSQL){
        foreach($matrizSQL as $array){
            if($array["mail"] == $mail && $array["clave"] == $clave){
                return true;
            }
        }
        return false;
